...
// =======================================================================================================================================
function subjectAjaxsearch() {
    var searchString = $('#subjectAjaxsearch').val();
            ...
            if (response['searchResultList']['data'].length > 0) {
                for (var i = 0; i < response['searchResultList']['data'].length; i++) {
                    var addStudent = '<tr class="in-queue">\
                        <td style="background-color:white;">'+ response['searchResultList']['data'][i]['subjectID'] + '</td>\
                    <td style="background-color:white;">'+ response['searchResultList']['data'][i]['subjectName'] + '</td>\
                    <td style="background-color:white;" class="details-control">\
                    <a sId="'+ response['searchResultList']['data'][i]['id'] + '" class="del ml-0" title="Remove"  onclick="deleteSubject(this);"><i class="fa fa-trash"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\
                         <a href="edit-subject/'+ response['searchResultList']['data'][i]['id'] + '" class="edit"><i class="fa fa-edit"></i></a>\
                    </td>\
                </tr>';

                    filteredData = filteredData + addStudent;
                }
                $('#subjectAppendData').html('');
                $('#subjectAppendData').append(filteredData);
            } else {
                $('#subjectAppendData').html('');
                $('#subjectAppendData').html('<tr>\
...
function studentpaginatorNextButton() {
    ...
    var queryString = $('#subjectAjaxsearch').val();
...
function deleteSubject(thisTxt) {
    var id = $(thisTxt).attr('sId');
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    Swal.close();
                    if(response['data']['message'] == 'Subject Attched with Someone'){
                        Swal.fire({
                            icon: 'error',
                            title: '<small>Subject Attched with Some other module</small>',
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: '<small>Subject deleted successfully</small>',
                            showConfirmButton: false,
                            timer: 2000
                        }).then(function(){
