...
// =======================================================================================================================================
function courseAjaxsearch() {
    var searchString = $('#courseAjaxsearch').val();
            ...
            if (response['searchResultList']['data'].length > 0) {
                for (var i = 0; i < response['searchResultList']['data'].length; i++) {
                    var addStudent = '<tr class="in-queue">\
                        <td style="background-color:white;">'+ response['searchResultList']['data'][i]['courseID'] + '</td>\
                    <td style="background-color:white;">'+ response['searchResultList']['data'][i]['courseType']['courseTypeName'] + '</td>\
                    <td style="background-color:white;">'+ response['searchResultList']['data'][i]['courseName'] + '</td>\
                    <td style="background-color:white;" class="details-control">\
                    <a cId="'+ response['searchResultList']['data'][i]['id'] + '" class="del ml-0" title="Remove" onclick="deleteCourse(this)"><i class="fa fa-trash"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\
                         <a href="edit-course/'+ response['searchResultList']['data'][i]['id'] + '" class="edit"><i class="fa fa-edit"></i></a>\
                    </td>\
                </tr>';

                    filteredData = filteredData + addStudent;
                }
                $('#courseAppendData').html('');
                $('#courseAppendData').append(filteredData);
            } else {
                $('#courseAppendData').html('');
                $('#courseAppendData').html('<tr>\
...
function studentpaginatorNextButton() {
    ...
    var queryString = $('#courseAjaxsearch').val();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response['data']['message'] == 'Course  Attched with Someone'){
                        Swal.fire({
                            icon: 'error',
                            title: '<small>Course Attched with Some other module</small>',
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: '<small>Course deleted successfully</small>',
                            showConfirmButton: false,
                            timer: 2000
                        }).then(function(){
