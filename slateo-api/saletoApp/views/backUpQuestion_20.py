        ...
        if data['questionType'].strip() == 'Subjective':
            ...
            for lang in languageList:
                ...
                try:
                    ...
                    if data_question:
                        checkFlag = True
                        existingQuestionID.append(data_question.id)
                    ...
                    questionContainer.append(data_question)
                count = count + 1

            if checkFlag:
                return Response({'message':'Question Already Exist','data':existingQuestionID}, status=409)
            else:
                for i in range(len(questionContainer)):
                    questionContainer[i].save()
                    data_question = Question.objects.get(refrence_Questions_Type_Detail=data_type_question,sameRefrenceUniqueCode=refrenceUniuecode,question=ques,optionList='')
            ...
            try:
                ...
                return Response({'message':'Something Went Worng, Try Again After Some Time'}, status=200)
